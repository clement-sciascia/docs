...
[OVH Dedicated Servers](https://www.ovh.com/asia/dedicated-servers/){.external} are easily able to boot into a Linux operating system using an updated kernel with the netboot startup system. However, it's best practice to update the kernel on the local machine.

**This guide will show you how to update the kernel for distributions that use an OVH core.**

> [!warning]
>
> By default, all system snapshots offered on [OVH Dedicated Servers](https://www.ovh.com/asia/dedicated-servers/){.external} use an optimised OVH core. If you have replaced these snapshots with your own distribution, please refer to your distribution’s official documentation.
>
...
- an [OVH Dedicated Server](https://www.ovh.com/asia/dedicated-servers/){.external}
...
In the event that you make a mistake or receive an error, it's possible to rollback your changes. To do so, the server must be placed in [Rescue mode](https://docs.ovh.com/asia/en/dedicated/ovh-rescue/){.external}. This will require you to mount your system using the following commands:
